...
{
    public partial class BudgetProjekSetup : PageHelper
    {
        MasterUser AuthUser;
        bool IsPreparer;
        bool IsReviewer;
        ...
        private void GetData()
        {
            try
            {
                ...
                List<BudgetProjek> BudgetData = new BudgetProjekBAL().GetBudgetProjekWithTreeCalc(LstSegmentDetailIDs, ref CanEdit)
                ...
                List<int> BlockedYears = new BudgetProjekBAL().GetBlockedProjekYears();
                Session["BlockedYears"] = BlockedYears;

                List<int> OpenYears = new BudgetProjekBAL().GetOpenProjekYears();
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    for (int c = index; c < gvAccountCodes.Columns.Count; c++)
                    {
                        ...
                        if (rowItem.ChildCount == 0 && IsBudgetEditable)
                        {
                            ...
                            tb.Text = (ObjBudgetProjek != null) ? ObjBudgetProjek.Amount.ToString("#,##0.00") : Convert.ToDecimal(0).ToString("#,##0.00"); ;
                            //lbl.Text = (ObjBudgetProjek != null) ? ObjBudgetProjek.Amount.ToString("#,##0.00") : Convert.ToDecimal(0).ToString("#,##0.00"); ;
                            ...
                            if (PeriodMenguruID == Convert.ToInt32("1"))
                            {
                                if (objpm != null)
                                {
                                    BudgetProjek getamount = BudgetData.Where(x => x.AccountCode == rowItem.AccountCode && x.PeriodMengurusID == objpm.PeriodMengurusID
                            ...
                            if (IsPreparer)
                            {
                                if (ObjBudgetProjek == null || ObjBudgetProjek.Status == "O" || ObjBudgetProjek.Status == "S" || ObjBudgetProjek.Status == "X" || ObjBudgetProjek.Status == "Y")
                                {
                                ...
                                else if (ObjBudgetProjek.Status == "P" || ObjBudgetProjek.Status == "R" || ObjBudgetProjek.Status == "A")
                                {
                                    tb.Visible = false;
                                    lbl.Visible = true;
                                    btn.Visible = false;
                            ...
                            if (IsReviewer)
                            {
                                if (ObjBudgetProjek == null || ObjBudgetProjek.Status == "O" || ObjBudgetProjek.Status == "S" || ObjBudgetProjek.Status == "P" || ObjBudgetProjek.Status == "X" || ObjBudgetProjek.Status == "Y")
                                {
                                    tb.Visible = ((pm.MengurusYear > DateTime.Now.Year) && !BlockedYears.Contains(pm.MengurusYear) && OpenYears.Contains(pm.MengurusYear));
                                    lbl.Visible = !((pm.MengurusYear > DateTime.Now.Year) && !BlockedYears.Contains(pm.MengurusYear) && OpenYears.Contains(pm.MengurusYear));
                                    if (ObjBudgetProjek != null && ObjBudgetProjek.Status == "P")
                                        btn.Visible = true;
                                    else
                                        btn.Visible = false;
                                        //btn.Visible = true;
                                }
                                else if (ObjBudgetProjek.Status == "R" || ObjBudgetProjek.Status == "A")
                                {
                                    tb.Visible = false;
                                    lbl.Visible = true;
                                    btn.Visible = false;
                            ...
                            if (IsApprover)
                            {
                                tb.Visible = false;
                                lbl.Visible = true;
                                if (ObjBudgetProjek != null && ObjBudgetProjek.Status == "R")
                                    btn.Visible = true;
                                else
                                    btn.Visible = false;
                            }
                                ...
                                foreach (AccountCode t in AccountCodesData.Where(x => ChildIDs.Contains(x.AccountCode1)))  //&& x.ChildCount == 0
                                {
                                    ...
                                    if (IsPreparer)
                                    {
                                        cnt = cnt + BudgetData.Where(x => x.AccountCode == t.AccountCode1 && x.PeriodMengurusID == PeriodMenguruID
                                            ...
                                            .Select(x => x.BudgetProjekID).Count();
                                    }
                                    if (IsReviewer)
                                    {
                                        cnt = cnt + BudgetData.Where(x => x.AccountCode == t.AccountCode1 && x.PeriodMengurusID == PeriodMenguruID
        ...
        protected void objTemp_OnCustomTextChanged(object sender, CustomEvenArgs e)
        {
            try
            {
                ...
                BudgetProjek item = BudgetData.Where(x => x.AccountCode == strAccountCode && x.PeriodMengurusID == PeriodMenguruID).FirstOrDefault();
                ...
                if (item != null)
                {
                    ...
                    BudgetProjek newBudgetProjek = new BudgetProjek();
                    newBudgetProjek.AccountCode = strAccountCode;
                    newBudgetProjek.PeriodMengurusID = PeriodMenguruID;
                    newBudgetProjek.Amount = amount;
                    newBudgetProjek.Status = strstatus;
                    newBudgetProjek.ModifiedBy = AuthUser.UserID;
                    newBudgetProjek.ModifiedTimeStamp = DateTime.Now;

                    List<int> LstSegmentDetailIDs = ((List<JuncBgtProjekSegDtl>)Session["ListSegmentDetails"]).Select(x => x.SegmentDetailID).ToList();
                    if(new BudgetProjekBAL().UpdateBudgetProjek(newBudgetProjek, LstSegmentDetailIDs))
                ...
                else
                {
                    ...
                    newBudgetProjek.PeriodMengurusID = PeriodMenguruID;
                    newBudgetProjek.Amount = amount;
                    newBudgetProjek.Status = (IsPreparer) ? new Helper().GetBudgetItemStatusEnumValue(Helper.BudgetItemStatus.Saved)
                        : new Helper().GetBudgetItemStatusEnumValue(Helper.BudgetItemStatus.Reviewed);
                    newBudgetProjek.CreatedBy = AuthUser.UserID;
                    newBudgetProjek.CreatedTimeStamp = DateTime.Now;
                    newBudgetProjek.ModifiedBy = AuthUser.UserID;
                    newBudgetProjek.ModifiedTimeStamp = DateTime.Now;

                    BudgetData.Add(newBudgetProjek);

                    List<JuncBgtProjekSegDtl> lstBgtSegDtl = (List<JuncBgtProjekSegDtl>)Session["ListSegmentDetails"];
                    foreach (JuncBgtProjekSegDtl obj in lstBgtSegDtl)
                        obj.BudgetProjek = newBudgetProjek;

                    if(new BudgetProjekBAL().InsertBudgetProjek(newBudgetProjek, lstBgtSegDtl))
        ...
        private List<JuncBgtProjekSegDtl> GetSegDetals()
        {
            List<JuncBgtProjekSegDtl> lst = new List<JuncBgtProjekSegDtl>();
            try
            {
                foreach (GridViewRow gvr in gvSegmentDLLs.Rows)
                {
                    ...
                    if (tv != null)
                    {
                        JuncBgtProjekSegDtl obj = new JuncBgtProjekSegDtl();
                        try
                        {
                        ...
                        {
                            return (List<JuncBgtProjekSegDtl>)Session["ListSegmentDetails"];
                        }
                        //obj.BudgetProjek = BudgetProjek;
                        lst.Add(obj);
                    }
                }
            }
        ...
        protected void btnDecisionSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (item != null)
                {
                    ...
                    newBudgetProjek.Amount = amount;
                    newBudgetProjek.Status = strstatus;
                    newBudgetProjek.Remarks = tbRemarks.Text;
                    newBudgetProjek.ModifiedBy = AuthUser.UserID;
                    newBudgetProjek.ModifiedTimeStamp = DateTime.Now;
                    ...
                    //else
                    //    ((SiteMaster)this.Master).ShowMessage("Failure", "An error occurred while updating Budget");
                    new BudgetProjekBAL().UpdateBudgetProjek(newBudgetProjek, LstSegmentDetailIDs);
                }
        ...
        protected void btnBulkUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (new BudgetProjekBAL().UpdateMultipleBudgetProjek(LstSegmentDetailIDs, PeriodIDs, FromChar, ToChar, tbBulkRemarks.Text.Trim(), LoggedInUser))
                    ((SiteMaster)this.Master).ShowMessage("Success", "Budget updated successfully");
                else
                    ((SiteMaster)this.Master).ShowMessage("Failure", "An error occurred while updating Budget");
        ...
        private DataTable GetProjekDataTable()
        {
            ...
            try
            {
                ...
                List<PeriodMenguru> PeriodData = (List<PeriodMenguru>)Session["PeriodData"];
                List<BudgetProjek> BudgetData = ((List<BudgetProjek>)Session["BudgetData"]).ToList();  //.Where(x => x.Status == "A").ToList();
        ...
        protected void btnSample_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet ds = new DataSet();
                ds.Tables.Add(BudgetProjekImport());

                new ReportHelper().ToExcel(ds, "BudgetProjekSample.xls", ref Response);
            }
            catch (Exception ex)
            {
                ((SiteMaster)this.Master).ShowMessage("Error", "An error occurred", ex, true);
        ...
        protected DataTable BudgetProjekImport()
        {
            DataTable dt = new DataTable();
            DataColumn dc = new DataColumn();
            ...
            try
            {
                if (!string.IsNullOrEmpty(FileUpload1.PostedFile.FileName))
                {
                    ...
                    if (Path.GetExtension(FileName) == ".csv")
                    {
                        ...
                        dt = new ReportHelper().ValidateBudgetImport<BudgetProjekSetup>(ds, gvAccountCodes, ref lstErrors);
        ...
        public bool MatchingData(DataRow dt)
        {
            ...
            foreach (string dipohon in ag)
            {
                if (dt.Table.Columns.Contains(dipohon))
                {
                    ...
                    if (pd != null)
                    {
                        List<BudgetProjek> bp = BudgetData.Where(x => x.PeriodMengurusID == pd.PeriodMengurusID && x.AccountCode == Convert.ToString(dt["AccountCode"])).ToList();
                    ...
                    if (gvAccountCodes.Columns[c].HeaderText.Contains(ColumnName))
                    {
                        ...
                        if (val == AccountCode)
                        {
                            ...
                            try
                            {
                                ...
                                BudgetProjek newBudgetProjek = new BudgetProjek();
                                newBudgetProjek.AccountCode = AccountCode;
                                newBudgetProjek.PeriodMengurusID = PeriodMenguruID;
                                newBudgetProjek.Amount = amount;

                                List<BudgetProjek> BudgetData = ((List<BudgetProjek>)Session["BudgetData"]).Where(x => x.AccountCode == AccountCode && x.PeriodMengurusID == PeriodMenguruID).ToList();
